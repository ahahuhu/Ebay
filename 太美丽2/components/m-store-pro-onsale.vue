...
<!-- 		<view class="m-distance">
			<image @click="updateOnSale(rowData.id)" style="width:40upx;height: 40upx;"
				src="@/static/img/icon/shop_icon_reduce.png" mode="aspectFit"></image>
		</view> -->
		<view class="m-distance">
			<image @click="onItemClick(rowData.id)" style="width:40upx;height: 40upx;"
				src="@/static/icon/update.png" mode="aspectFit"></image>
		</view>
	...
	export default {
		...
		methods: {
			onItemClick() {
				...
				uni.navigateTo({
					url: `/pages/my/update_com_info?id=${productIddd}`
				});
			},
			// updateOnSale(id) {
			...
			// 	uni.request({
			// 		url: 'http://localhost:8016/deleteProduct',
			// 		method: 'GET',
			// 		data: {
			// 			userID: userID,
			// 			id: id
				...
				uni.request({
					...
					success: (res) => {
						if (res.data.code == 200) {
							...
							// 重新启动应用并跳转到当前页面
							uni.reLaunch({
								url: '/pages/my/controll' // 替换为当前页面的路径
							});
						} else {
							// 处理错误情况
