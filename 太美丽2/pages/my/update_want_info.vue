<template>
	<view class="container">
		...
		<button class="submit-button" @click="handleSubmit">修改</button>
	</view>
</template>
...
<script>
	export default {
		...
		onLoad(option) {
			this.goods.seekerId=option.seekerId
		    uni.request({
		    	url:'http://localhost:8016/getSeeker?seekerID='+this.goods.seekerId,
				method:'GET',
				success: (res) => {
					if(res.data.code==200){
					   ...
					   this.goods.address=res.data.result.address
					}
				}
		    })
		},
		methods: {
			...
			handleSubmit() {
				uni.request({
					url:"http://localhost:8016/updateSeeker",
					data:this.goods,
					method:"POST",
					success: (res) => {
						console.log(res)
